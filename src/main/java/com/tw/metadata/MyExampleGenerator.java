...
public class MyExampleGenerator {
    ...
    public Optional<Object> resolvePropertyToExample(Map<String, Object> examples, List<String> mediaTypes, Property property) {
        ...
        Map<String, String> generate = generate(examples, mediaTypes, property).get(0);
        if(generate.size() > 1) {
            return Optional.of(generate.get(EXAMPLE));
        }
        return Optional.empty();
    }

    private List<Map<String, String>> generate(Map<String, Object> examples, List<String> mediaTypes, Property property) {
...
//            added by zhengpeizhen
            ObjectProperty objectProperty = (ObjectProperty) property;
            Map<String, Property> objectPropertyProperties = objectProperty.getProperties();
            Object example = objectProperty.getExample();
            if( example != null) {
                return usingExampleFromSpec(example);
            }
            logger.debug("Creating example from object property");
            HashMap result = new HashMap();
            objectPropertyProperties.forEach((pName, pValue) -> result.put(pName, resolvePropertyToExample(pName, mediaType, pValue, processedModels)));
            objectProperty.setExample(result);
            return result;
    ...
    private Object resolveModelToExample(String name, String mediaType, Model model, Set<String> processedModels) {
        ...
        if (model instanceof ModelImpl) {
            ...
            return resolveModelImplToExample(mediaType, processedModels, impl);
        }
        return "";
    }

    private Object usingExampleFromSpec(Object example) {
        logger.debug("Using example from spec: {}", example);
        return example;
    }

    public Object resolveModelToExample(String mediaType, Model model, Set<String> processedModels) {
        if (model instanceof ModelImpl) {
            ModelImpl impl = (ModelImpl) model;

            if(logger.isDebugEnabled()) {
                logger.debug("Resolving model '{}' to example", Json.pretty(model));
            }
    ...
    private Object resolveModelImplToExample(String mediaType, Set<String> processedModels, ModelImpl impl) {
        Map<String, Object> values = new HashMap<>();
        Object result = null;
        if (impl.getExample() != null) {
            result =  usingExampleFromSpec(impl.getExample());
        } else if (impl.getProperties() != null) {
            logger.debug("Creating example from model values");
            impl.getProperties().forEach((pName, pValue) -> values.put(pName, resolvePropertyToExample(pName, mediaType, pValue, processedModels)));
